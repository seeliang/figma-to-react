 ...
 *
 * Shared by `fidelity.html`, which shows the whole frame at once, and by each
 * story's play function, which is what turns drift into a failing build.
 */
...
export const geometryById = geometry as Record<string, NodeBox>
 ...
 */
export function measure(container: HTMLElement | Document = document): Delta[] {
  ...
  const anchor = geometryById[root.dataset.figmaId!]
  if (!anchor) return []
  ...
  for (const el of scope.querySelectorAll<HTMLElement>('[data-figma-id]')) {
    ...
    // A COMPONENT's box is where its definition sits on the Figma canvas,
    // which says nothing about where an instance of it renders inside another
...
export async function expectLayoutWithin(
  ...
  const failures = measure(container).filter((d) => worstDelta(d) > thresholdPx)
